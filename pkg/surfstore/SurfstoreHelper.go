...
func LoadMetaFromMetaFile(baseDir string) (fileMetaMap map[string]*FileMetaData, allFileMetaMap map[string]bool, e error) {
	...
	if e != nil || metaFileStats.IsDir() {
		return fileMetaMap, allFileMetaMap, nil
	}
	...
	for {
		...
		_, inSet := allFileMetaMap[currFileMeta.Filename]
		if !inSet {
			allFileMetaMap[currFileMeta.Filename] = true
		}
	}
...
// Get hash list of a file
func GetFileHashList(baseDir, fileDir string, blockSize int) (hashList []string, err error) {
	// read file into byte list
	byteList, err := os.ReadFile(ConcatPath(baseDir, fileDir))
	if err != nil {
		return hashList, err
	}
	...
	for i := 0; i < len(byteList); i += blockSize {
		endIdx := i + blockSize
		if i+blockSize >= len(byteList) {
			endIdx = len(byteList) - 1
		}
		hashList = append(hashList, GetBlockHashString(byteList[i:endIdx]))
	}
	return hashList, nil
}
